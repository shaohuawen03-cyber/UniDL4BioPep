...
## 7b. 一键跑完剩余全部任务（32 核以内）

Macrel 单工具跑完之后，剩下的步骤一条命令跑完并组装成总结果：
...
| 步 | 内容 |
|---|---|
| 0 | 前置检查：两个预测目录在不在、探测有哪些队列 |
| 1 | （可选）UniDL4BioPep 单工具全量 —— 默认**跳过** |
| 2 | `amp_group_stats.py` 组间统计（每个结果目录） |
| 3 | `amp_ad_association.py` AD 关联（每个结果目录 × 每个队列，自动判断 `--min-tools`） |
| 4 | `compare_macrel_vs_unidl.py` 对比表 |
| 5 | `assemble_results.py` 组装总表 + 报告 |

产物在 `FINAL_REPORT/`：

```
FINAL_REPORT/
├── README_结果汇总.md      ← 先看这个, 可直接引用
├── 01_headline.tsv         两个工具的判阳率总表
├── 02_macrel_thresholds.tsv
├── 03_unidl_thresholds.tsv
├── 04_discriminative.tsv   阈值塌缩倍数(判别力)
├── 05_group_diff.tsv       组间差异(按效应量排序)
└── cmp_*.tsv               compare 脚本的三张表
```
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `CORES` | `8` | 并发核数，**硬上限 32**，超过自动压到 32；也受本机核数限制 |
| `CMP_MACREL_TH` | `0.9` | 第 4 步对比表里"认定 Macrel 命中"的阈值，**必须等于共识跑当时用的值**（你那次是 0.9）。注意它和 `MACREL_TH`（Macrel 那步自己的判阳阈值，默认 0.5）是两个东西 |
| `THRESHOLD` | `0.99` | 共识跑当时 UniDL4BioPep 的阈值 |
| `RUN_UNIDL_ONLY` | `0` | `1` = 额外单独跑一次 UniDL4BioPep（最慢的一步） |
| `FORCE` | `0` | `1` = 忽略"已完成"标记强制重跑 |

特性：每一步单独写日志到 `logs/`；**某步失败不中断**，继续跑下一步，
最后汇总哪些成了哪些没成。

> **默认不单独跑 UniDL4BioPep**，因为它的判阳率和阈值敏感性可以直接从
> 共识跑的 `summary.json` 里取（`n_hits.AMP` 和
> `prob_distribution.AMP.counts_at_threshold` 是分工具存的），
> 不需要重跑 ESM-2 + CNN。只有需要 UniDL4BioPep **单独的候选肽清单**时
> 才加 `RUN_UNIDL_ONLY=1`。

> **AD 关联对 Cohort2 只能做到组间比较**：Cohort2 只有 NC / AD 两个阶段，
> Cochran-Armitage 趋势检验需要 ≥3 个有序阶段，脚本会明确提示并跳过。
> 真正的阶段趋势要跑 Cohort1 或 Cohort3。
...
### Macrel 单工具跑（1.57 小时，同日）

比共识跑快 **2.8 倍**（1.57 h vs 4.43 h），因为不加载 ESM-2、不跑 CNN。

**交叉核对通过**：两次跑的 `n_raw`（24,813,729 / 23,347,543）和 `n_scored`
（8,136,428 / 7,703,149）逐位相同，且 **Macrel@0.9 = 2,461 / 2,416，与共识跑
记录的 Macrel 命中数完全一致** —— 说明两次跑的是同一批序列，`run_macrel_only.py`
的口径与原流程对得上。

Macrel 阈值敏感性（判阳率 vs 原始 smORF）：

| 阈值 | Disease_AD | Healthy_NC | 对文献基准 0.1–1.65% |
|---|---|---|---|
...
**Macrel@0.7 是唯一落在文献基准区间内的档位**，而且这个数是从已经跑完的
概率直方图里读出来的，不用重跑。
...
| | 阈值从最宽到最严 | 判阳率塌缩 |
|---|---|---|
| Macrel | 0.5 → 0.99 | 7.327% → 0.0000081%，**塌缩 909,034 倍** |
| UniDL4BioPep | 0.5 → 0.999 | 概率挤在高位，**阈值几乎拧不动** |

Macrel 的概率有区分度，阈值是有意义的旋钮；UniDL4BioPep 的概率挤在一起，
把阈值从 0.99 拧到 0.999 才刚开始起作用 —— 这就是"该工具在本数据上
不构成筛选"的直接证据，比单看一个判阳率更有说服力。
